...
import { Button, Descriptions, Form, Image, Input, Modal, Space, Table, Tag, Typography, message } from 'antd';
...
const PendingReviews = () => {
  ...
  const handleDelete = (review: Review) => {
    Modal.confirm({
      ...
      onOk: async () => {
        if (!review.store?.id) {
          message.error('无法删除，缺少店铺 ID');
          return;
        }
        try {
          await adminDeleteReview(review.store.id, review.id);
